...
namespace OcpSample
{
    public class ETicketInvoiceCreator : InvoiceCreator
    {
        public ETicketInvoiceCreator(InvoiceEngine engine)
            : base(engine)
        {
        }
        ...
        {
            _engine.TaxCalculationRate = 20m;
            _engine.ItemPrice = 10m;
            InvoiceDto invoice = InitializeSampleETicketInvoice();
            _engine.InvoiceSource.InsertInvoiceTypeToJson(_engine.JsonSerializer.GetJsonStringFromInvoice(invoice));
        }

        private InvoiceDto InitializeSampleETicketInvoice()
        {
            InvoiceDto invoice = new InvoiceDto()
            {
                ...
                InvoiceTypeCode = "ETicketInvoice",
                DocumentCurrencyCode = "TRY",
                Note = new List<string> { "Note 1", "Note 2" },
            };
            var invoiceLineType = new InvoiceLineTypeDto();
            invoiceLineType.Item = "Airline Ticket";
            invoiceLineType.Quantity = 1;
